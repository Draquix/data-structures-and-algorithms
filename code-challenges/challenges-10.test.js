...
const getNames = (arr) => {
  ...
  const arrmap = arr.map((value, index) => {
    ...
    newArray.push(reverseArray.join(""));
  })
  return newArray;
};
...
const totalSum = (input) => {
  ...
  let accTot = 0;
  input.forEach(arr => {
    arr.reduce( (accumulator, val) => accTot = accumulator + val, 0 );
    total += accTot;
  })
  return total;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  const answerArray = [];
  input.forEach( arr => {
    let newArr = arr.filter(num => (typeof num === 'number' && num % 5 ===0)).map(num => Math.pow(2,num));
...
let findShortest = (data) => {
  const heights = [];
  let name = "";
  data.forEach(obj => heights.push(obj.height));
  let shorty = Math.min.apply(Math, heights);
  console.log(shorty);
  data.forEach(obj => {
    if (obj.height == shorty){
      name = obj.name;
    }
  });
  return name;
